                                            ...
                                            {passwordArray.map((item, index) => (
                                                <tr key={index} className="bg-transparent border-b border-[#e99e14] hover:bg-[#1a1a1a]">
                                                    <th scope="row" className="px-4 py-3 font-medium text-[#ffffff] whitespace-nowrap max-w-[220px] truncate">
                                                        ...
                                                        <button onClick={() => copyText(item.site)} aria-label={`Copy site ${item.site}`} className="inline-block ml-2 align-middle"                                                        >
                                                    ...
                                                    <td className="px-4 py-3 max-w-40 truncate">
                                                        ...
                                                        <button onClick={() => copyText(item.username)} aria-label={`Copy username ${item.username}`} className="inline-block ml-2 align-middle" >
                                                            <img src={copy} alt="Copy" className="h-4 w-4" />
                                                        </button>
                                                    </td>

                                                    <td className="px-4 py-3 max-w-40 truncate">
                                                        <span className="inline-block align-middle truncate">{item.password}</span>
                                                        <button onClick={() => copyText(item.password)} aria-label={`Copy password for ${item.site}`} className="inline-block ml-2 align-middle" >
                                                            <img src={copy} alt="Copy" className="h-4 w-4" />
                                                        </button>
                                                    </td>

                                                    <td className="px-4 py-3 text-right space-x-2">
                                                        <button className="inline-block" onClick={() => editPassword(item.id)} aria-label={`Edit ${item.site}`} >
                                                            <img src={edit} alt="Edit" className="h-5 w-5" />
                                                        </button>
                                                        <button className="inline-block" onClick={() => deletePassword(item.id)} aria-label={`Delete ${item.site}`} >
